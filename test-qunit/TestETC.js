...
test("Test - Timezone Conversion", function() {
    ...
    var results = chrono.parse("today at 4:30 pm (Europe/Berlin)", 'Thu Dec 19 2013 12:00:00');
    ...
    ok(result.start.hour == 16, 	'hour in ' +JSON.stringify( result.start ) );
    ok(result.start.minute == 30, 	'year in ' +JSON.stringify( result.start ) );
    ok(result.start.timezoneRegion == "Europe/Berlin", result.start.timezoneRegion);


    var results = chrono.parse("today at 4:30 pm EST", 'Thu Dec 19 2013 12:00:00');
    ...
    ok(result.start.hour == 16, 	'hour in ' +JSON.stringify( result.start ) );
    ok(result.start.minute == 30, 	'year in ' +JSON.stringify( result.start ) );
    ok(result.start.timezoneRegion == "EST", result.start.timezoneRegion);
